...
# Lexer executable paths and test directories
if [ "$ENGLISH" = true ]; then
    TEST_DIRS=("test/questions_en" "test/questions_advanced_en")
    EXPECTED_DIR="test/answers_en"
else
...
for dir in "${TEST_DIRS[@]}"; do
    ...
    # Scoring logic
    POINTS_PER_TEST=2
    if [[ "$dir" == *"策問"* ]] || [[ "$dir" == *"questions_en"* ]]; then
        POINTS_PER_TEST=8
    fi
